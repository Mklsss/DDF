"""One-batch forward/backward validation for the complete DDF Mixed model."""
...
from mixed_experiment import DDFMixed, THIS_DIR, load_config, make_loader, parameter_count, set_seed
...
def main():
    ...
    parser.add_argument("--output", default=str(THIS_DIR / "results" / "smoke_mixed_ddf_S12_B3.json"))
    args = parser.parse_args()
    config, config_path = load_config(args.config)
    set_seed(int(config["seed"]))
    device = torch.device(args.device)
    sinogram, target = next(iter(make_loader(config["train_data"], args.sparse_factor, args.batch_size)))
    model = DDFMixed(config).to(device).train()
    ...
    report = {
        ...
        "architecture": "ddf-Mixed-P-Swin-REDCNN",
        ...
        "pswin_has_finite_gradient": has_finite_gradient(model.sin),
        "redcnn_has_finite_gradient": has_finite_gradient(model.ct),
        "pswin_parameters": parameter_count(SwinIRSino(config["pswin"])),
