...
def init_swanlab(args, method, sparse_factor, config):
    ...
    try:
        ...
        raise RuntimeError(
            "swanlab is not installed. Install it first, or rerun without --use_swanlab."
        ) from exc

    run_name = args.swanlab_run_name or f"{method}_S{sparse_factor}"
    return swanlab.init(
        ...
        config={
            ...
            "config_path": str(args.config or Path(__file__).with_name("config_default.json")),
        },
    )
...
class FbpLayer(nn.Module):
    def __init__(self, matrix_path):
        ...
        # Recreate the fixed operator from disk instead of serializing it into
        # every checkpoint.  Sparse operator persistence produced very large
        # checkpoints that fail PyTorch's sparse-tensor load validation.
...
class SimpleSineFusion(nn.Module):
    ...
    def forward(self, feedback_sinogram, utr_sinogram):
        return feedback_sinogram + utr_sinogram
...
def train_model(method, sparse_factor, config, output_checkpoint, batch_size, device, swanlab_run=None):
    model = build_model(method, sparse_factor, config).to(device)
    ...
    for epoch in range(int(config["train"]["epochs"])):
        ...
        progress = tqdm(
            train_loader,
            desc=f"train {method} S={sparse_factor} epoch {epoch + 1}/{int(config['train']['epochs'])}",
            leave=False,
        )
        for step_index, (sparse_sinogram, target_ct) in enumerate(progress, start=1):
            sparse_sinogram_device = sparse_sinogram.to(device=device, dtype=torch.float32)
            target_ct_device = target_ct.to(device=device, dtype=torch.float32)
            prediction_ct, _ = model(sparse_sinogram_device)
                ...
                swanlab_run.log(
                    {
                        ...
                        "train/epoch": epoch + 1,
                        "train/step": global_step_base + step_index,
                    }
                )
        scheduler.step()
        ...
        print(
            f"epoch={epoch + 1}, sparse_factor={sparse_factor}, "
            f"train_loss={mean_loss:.6f}, psnr={psnr_value:.6f}, ssim={ssim_value:.6f}"
        )
        if swanlab_run is not None:
            swanlab_run.log(
                {
...
def common_parser(description):
    ...
    parser.add_argument("--use_swanlab", action="store_true", help="Log training metrics to SwanLab.")
    parser.add_argument("--swanlab_project", default="DDF-ablation", help="SwanLab project name.")
    parser.add_argument("--swanlab_run_name", default=None, help="Optional SwanLab run name.")
    return parser
...
def run_fixed_factor_experiment(args, method, method_key, display_name, csv_name):
    ...
    for sparse_factor in parse_factors(args.sparse_factor):
        ...
        if args.mode == "train":
            ...
            swanlab_run = init_swanlab(args, method_key, sparse_factor, config)
            try:
                train_model(method, sparse_factor, config, checkpoint_path, args.batch_size, device, swanlab_run=swanlab_run)
            finally:
                if swanlab_run is not None:
                    swanlab_run.finish()
