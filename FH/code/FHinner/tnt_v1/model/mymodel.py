...
class mymodel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(2, 1)
        x_cat = torch.zeros((B, 42, angle)).to(device)  # 3*42*180

        xlay1_1 = self.sin_sensor_lay1(x)  # 3*520*180
        x_up = torch.cat((x_cat, xlay1_1, x_cat), dim=1)  # 3*604*180
        xlay1_1 = torch.cat((x_up[:, 0:302, :], x[:, 178, :].unsqueeze(1), x_up[:, 302:, :]), dim=1)  # 3*605*180
        ...
        x_cat = torch.zeros((B, 42, 360)).to(device)
        x_up = torch.cat((x_cat, xlay2_2, x_cat), dim=1)
        xlay2_2 = torch.cat((x_up[:, 0:302, :], xlay1_2[:, 178, :].unsqueeze(1), x_up[:, 302:, :]), dim=1)
